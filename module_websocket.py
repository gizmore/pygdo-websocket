...
class module_websocket(GDO_Module):
    ...
    def gdo_module_config(self) -> list[GDT]:
        return [
            ...
            GDT_Port('ws_port').not_null().initial('61551'),
            GDT_Bool('ws_tls').not_null().initial('0'),
            GDT_Path('ws_tls_key').existing_file(),
            GDT_Path('ws_tls_cert').existing_file(),
            GDT_Host('ws_client_host').not_null().initial(Application.config('core.domain')),
            GDT_Port('ws_client_port').not_null().initial('61555'),
            GDT_Bool('ws_client_tls').not_null().initial('1'),
    ...
    def cfg_tls_cert_path(self) -> str:
        return self.get_config_val('ws_tls_cert')

    def cfg_client_host(self) -> str:
        return self.get_config_val('ws_client_host')

    def cfg_client_port(self) -> int:
        return self.get_config_value('ws_client_port')

    def cfg_client_tls(self) -> bool:
        return self.get_config_value('ws_client_tls')
    ...
    def gdo_init_sidebar(self, page: 'GDT_Page'):
        self.add_js_inline("window.gdo.ws.tls = " + str(int(self.cfg_client_tls())) +";\nwindow.gdo.ws.ip = '" + self.cfg_client_host() + "';\nwindow.gdo.ws.port = " + str(self.cfg_client_port()) + ";\nwindow.gdo.ws.cookie = '" + Application.get_cookie(GDO_Session.COOKIE_NAME) + "';")
